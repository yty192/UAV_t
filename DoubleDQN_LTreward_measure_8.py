...
class Agent:
    ...
    def replay(self, batch_size):
        ...
        index = 0
        target_network_update = False
        for state, action, reward, next_state in minibatch:
            if self.number_reply % 50 == 0:
                target_network_update = True
    ...
    if method == 10:
        ...
        for l in range(episodes):
            ...
            for m in range(counter):
                ...
                tar_net_update = False
                if len(agent.memory) > batch_size:
                    tar_net_update = agent.replay(batch_size)

                if tar_net_update is True:   # for test
                    print('%%%%%%%%%%%*************')
            ...
            list_NN_success_total_bit.append([r_total_size])   # figure 10
