...
class Agent:
    def __init__(self, action_size1, network_model):
        ...
        self.epsilon_number = 150 * 20
        self.epsilon_decay = self.epsilon_min ** (1 / self.epsilon_number)
    ...
    def replay(self, batch_size):
        ...
        # list_reward = [mm[2] for mm in self.memory]
        # print(list_reward)
        ...
        for state, action, reward, next_state in minibatch:
            ...
            # index_rewards = index_reward
            # while index_rewards > 0:
            #     index_rewards -= 1
            #     reward += self.memory[index_rewards]
            # reward = reward/(index_reward-index_rewards+1)

            model_values = self.model.predict(next_state)  # DDQN
            max_action_next = np.argmax(model_values[0])
            model_target_values = self.model_target.predict(next_state)[0]
