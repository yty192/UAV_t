...
from keras.layers import Dense, InputLayer, Flatten, Conv1D, Activation, LeakyReLU, Add, Subtract, Lambda, Input
...
class Users:
    ...
    def initial_coordinate(self):   #initiae users in Ring
...
class Environment:
    ...
    def list_update(self, j1, total_size1, list_in2):    # update the list UAV covered
        list_now1 = []
    ...
    def judgement(self, list_now2, process_number2, package_task2):  # When a submission is accomplished,
        task_success1 = False                                        # judge whether this user is still
    ...
    def judge(self, list_now3, process_number1, package_task1):    # When the submission is not finished,
        in_list = 0                                               # judge whether this user is still in the coverage.
        not_in_list = 0
        aa = 0
    ...
    def ratio_result(self, sum_package2):      # results for plotting
    ...
    def getState(self, list_now10, action_size10):    # form the state
...
class Agent:
    ...
    def _build_NN_model(self):      # NN
        inputs = Input(shape=(density_value,2 ))
        _x = Flatten()(inputs)
        _x = Dense(128, activation='relu')(_x)
        _x = Dense(128, activation='relu')(_x)
        _x = Dense(128, activation='relu')(_x)
        value = Dense(1, activation='linear')(_x)
        adv = Dense(self.action_size, activation='linear')(_x)
        _meam = Lambda(lambda _x: K.mean(_x, axis=1, keepdims=True))(adv)
        advantage = Subtract()([adv, _meam])
        q_value = Add()([value, advantage])
        model = Model(inputs=inputs, outputs=q_value)
        model.compile(loss='mse', optimizer=Adam(lr=self.learning_rate))
        return model

    def _build_Con_model(self):       # CNN
    ...
    # def remember(self, state, action, reward, next_state):    for non_PER method
    #     self.memory.append((state, action, reward, next_state))

    def store_transition(self, state, action, reward, next_state):   # Memory
    ...
    def act(self, list_now4):
        ...
        if method == 0:
            ...
            index_distance = list_distance.index(min(list_distance))     # the nearest: +distance min
            return index_distance
        elif method == 2:
            list_package = [p[4] for p in list_now4]                   # the smallest size of package
            index_package = list_package.index(min(list_package))
            return index_package
        elif method ==3:
            list_distance_n = [dn[3] for dn in list_now4]
            index_distance_n = list_distance_n.index(min(list_distance_n))    # the farthest user behind UAV
            return index_distance_n
        elif method ==4:
            list_distance_p = [dp[3] for dp in list_now4]
            index_distance_p = list_distance_p.index(max(list_distance_p))    # the farthest user before UAV
    ...
    def replay(self, batch_size):
        ...
        for r in range(batch_size):
            ...
            model_values = self.model.predict(next_states_)           # DDQN
            max_action_next = np.argmax(model_values[0])
            model_target_values = self.model_target.predict(next_states_)[0]
            target = reward + self.gamma * model_target_values[max_action_next]
        ...
        abs_errors = np.sum(np.abs(targets - targets_f), axis=1)
        #print('abs_errors is', abs_errors)
        self.loss = np.sum(ISWeights * np.square(targets - targets_f))
...
class Memory(object):  # used in PER, stored as ( s, a, r, s_ ) in SumTree
    ...
    def sample(self, n):    # n is batch_size
        ...
        non_zero = self.tree.tree[-self.tree.capacity:]
        non_zero_index = np.nonzero(non_zero)
        min_prob = np.min(non_zero[non_zero_index]) / self.tree.total_p     # for later calculate ISweight
...
class SumTree(object):      # the method used in PER
    data_pointer = 0
...
if __name__== "__main__":
    ...
    episodes = 200             #### for method 10:  episodes=10:memory=292(around)
    ...
    if method == 10:
        ...
        #random.seed(11)   # random the different data every episodes
        env = Environment()
        user = Users()
        agent = Agent(density_value, network_Model)

        for l in range(episodes):      # for every circle
            ...
            for m in range(counter):    # for every update time
